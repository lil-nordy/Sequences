...
def print_all_items_backwards(sequence):
    ...
    for k in range(len(sequence) -1, -1, -1):
        # starts at the end of the array, which is the length - 1 [1..8] has len = 8, but last item is indexed at 7
        # since it starts at 0...stops when it hits minus 1...or the index at zero..
        print(sequence[k])
...
def print_items_that_are_bigger_than_5(sequence):
    ...
    for k in range(len(sequence)):
        # if sequence[k] > 5:
        #     print('%(digit)d is at index %(index)d' % {'digit': sequence[k], 'index': k})
        if sequence[k] > 5:
...
def print_items_that_are_odd_integers(sequence):
    ...
    # ------------------------------------------------------------------
    for k in range(len(sequence)):
        if type(sequence[k]) == int & sequence[k] % 2 != 0:
            print('{} is at index {}'.format(sequence[k], k))
        # my peer Aman did this a bit differently:
        # if sequence[k].__class__ == int & sequence[k] % 2 !=0:
        #     print('{} is at index {}'.format(sequence[k], k))
